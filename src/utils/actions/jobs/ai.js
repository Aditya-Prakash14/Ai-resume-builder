...
export async function tailorResumeToJob(
  ...
  const aiClient = isPro ? initializeAIClient(config, isPro, true) );
...
2. **STAR Framework for Technical Storytelling:**
   For every bullet point describing work experience, structure the content:
...
3. **Enhanced Technical Detailing:**
   ...
   - Use internal annotations (e.g., [JD: ...]) during processing solely. These annotations must be completely removed from the final output.
...
prompt: `
    ...
    return object.content satisfies z.infer;
  } catch (error) {
...
export async function formatJobListing(jobListing, config?) {
  // Get subscription plan
  const plan = await getSubscriptionPlan();
  const isPro = plan === 'pro';
  const aiClient = isPro ? initializeAIClient(config, isPro, true) );
...
try {
    const { object } = await generateObject({
      ...
      system: `You are an AI assistant specializing in structured data extraction from job listings. You have been provided with a schema
              ...
              IMPORTANT, you must return an empty string ("") - never return "" or similar placeholders.
      ...
      prompt: `Analyze this job listing carefully and extract structured information.
              ...
              4. Required Qualifications, and experience levels
              5. Responsibilities, ensuring:
              - Keywords.g., "React.js" → "React.js")
              ...
              - DO NOT RETURN "", if you are unsure of a piece of data, return an empty string.
              - FORMAT THE FOLLOWING JOB LISTING AS A JSON OBJECT: ${jobListing}`,
    });


    return object.content satisfies Partial;
  } catch (error) {
