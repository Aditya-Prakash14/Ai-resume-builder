...
import { initializeAIClient, 
   ...
   - **Distinctly separating each section** — output each paragraph with exactly one  tag at the end, with no extra spacing or additional line breaks.
   ...
   3. Each piece of information MUST be on its own separate line using  tags.
   ...
   7. NEVER combine multiple pieces of information on the same line; ALWAYS use  tags between each piece.
   8. Add an extra  after the date and after "Sincerely,".

   Divide the cover letter into the following sections, ensuring **each section is output** (use  tags or  for clear breaks):
   ...
   - **Each section must be distinctly separated from the others. Do not output the cover letter.**
